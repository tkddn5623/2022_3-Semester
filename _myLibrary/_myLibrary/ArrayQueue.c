...
ArrayQueue* AQ_newQueue(const int max) {
	...
	if ((pqueue->items = malloc((max + 1) * sizeof(Element))) == NULL) exit(1);
...
Element AQ_front(const ArrayQueue* pqueue) {
	...
	if (front == pqueue->rear) exit(1);
	return pqueue->items[front];
}
Element AQ_back(const ArrayQueue* pqueue) {
	const int rear = pqueue->rear, capacity = pqueue->capacity;
	if (pqueue->front == rear) exit(1);
	return pqueue->items[(rear + capacity - 1) % capacity];
}
void AQ_push(ArrayQueue* pqueue, const Element item) {
	const int rear = pqueue->rear, capacity = pqueue->capacity;
	if (pqueue->front == (rear + 1) % capacity) exit(1);
	pqueue->items[rear] = item;
	pqueue->rear = (rear + 1) % capacity;
}
